...
class StoreBackend(BackendProtocol):
    ...
    def __init__(self, runtime: "ToolRuntime"):
        ...
        if not _LANGGRAPH_AVAILABLE:
            raise ImportError(
                "langgraph is required for StoreBackend. Install it with: pip install langgraph"
            )
    ...
    def ls_info(self, path: str) -> list[FileInfo]:
        ...
        Returns:
            List of FileInfo objects for files and directories directly in the directory.
            Directories have a trailing / in their path and is_dir=True.
        """
        store = self._get_store()
        namespace = self._get_namespace()

        # Retrieve all items and filter by path prefix locally
    ...
    def grep_raw(
        ...
        for item in items:
            try:
                files[str(item.key)] = self._convert_store_item_to_file_data(item)
            except ValueError:
                continue
